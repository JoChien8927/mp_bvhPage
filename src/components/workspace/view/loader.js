...
class XNectLoader extends THREE.Loader {
  ...
  parse(txt) {
    ...
    for (const humanPair of humans) {
      ...
      if (!poses.has(humanPair[0]))
        poses.set(humanPair[0], { skeleton: skeleton, clip: clip });
    }
  ...
  parseSkeleton(humanPair) {
    ...
    for (const framePair of humanPair[1]) {
      ...
      for (const i of Array(bonesNum).keys())
        bones[i].position.copy(framePair[1][i]);

      for (const i of Array(bonesNum).keys()) {
        const parent = bones[i].parent;

        localPositions[i] = parent
          ? framePair[1][i].clone().sub(parent.position)
          : framePair[1][i];
      }
    ...
    for (const i of Array(bonesNum).keys())
      bones[i].position.copy(humanPair[1].values().next().value[i]);
