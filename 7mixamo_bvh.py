...
def process_file(keypoints_input, outputfile, fps):
    ...
    predictions_copy = np.zeros((num_frames, len(new_joint_list), 3))

    # 獲取 joints 的名稱映射
    name_idx_map = get_name_idx_map(landmark_names)

    for frame_idx, frame_data in enumerate(frames_data):
        frame_keypoints = {kp["name"]: np.array(kp["coordinates"]) for kp in frame_data["keypoints"]}
        
        for index, joint_name in enumerate(new_joint_list):
            # 使用 map_mediapipe_to_mixamo 函數來計算每個關節的位置
            position = map_mediapipe_to_mixamo(joint_name, frame_keypoints, name_idx_map)
            predictions_copy[frame_idx][index][0] = position["x"]
            predictions_copy[frame_idx][index][1] = position["y"]
            predictions_copy[frame_idx][index][2] = position["z"]
...
def write_mediapipe_bvh(output_file, prediction3dpoint, fps):
    bvhfileName = output_file
    skeleton = mediapipe_skeleton.MediapipeSkeleton()
    
    # 构建新的层级结构
    skeleton.set_hierarchy(new_joint_list)
...
def map_mediapipe_to_mixamo(bone_name, frame_keypoints, name_idx_map):
    ...
    # 需特殊計算的關節
    Hips = {
            "x": (frame_keypoints["LeftHip"][0] + frame_keypoints["RightHip"][0]) / 2 *50,
            "y": (frame_keypoints["LeftHip"][1] + frame_keypoints["RightHip"][1]) / 2 *50,
            "z": (frame_keypoints["LeftHip"][2] + frame_keypoints["RightHip"][2]) / 2 *50
        }
    Neck = {
        "x": (frame_keypoints['LeftShoulder'][0] + frame_keypoints['RightShoulder'][0]) / 2 *50,
        "y": (frame_keypoints['LeftShoulder'][1] + frame_keypoints['RightShoulder'][1]) / 2 *50,
        "z": (frame_keypoints['LeftShoulder'][2] + frame_keypoints['RightShoulder'][2]) / 2 *50
    }
    # Spine1 @ Hips 和 Neck 之間
    #  因為 Hips 與 Neck 不存在於 Mediapipe 的關鍵點中，所以需要自己計算 並取上方計算點的平均值
    Spine1 = {
        "x": (Hips["x"] + Neck["x"]) / 2,
        "y": (Hips["y"] + Neck["y"]) / 2,
        "z": (Hips["z"] + Neck["z"]) / 2
    }
    # Spine2 @ Spine1 和 Neck 之間
    Spine2 = {
        "x": (Spine1["x"] + Neck["x"]) / 2,
        "y": (Spine1["y"] + Neck["y"]) / 2,
        "z": (Spine1["z"] + Neck["z"]) / 2
    }
    # Spine @ Spine1 和 Hips 之間
    Spine = {
        "x": (Spine1["x"] + Hips["x"]) / 2,
        "y": (Spine1["y"] + Hips["y"]) / 2,
        "z": (Spine1["z"] + Hips["z"]) / 2
    ...
    if bone_name == 'mixamorig:Hips':
        ...
        if mediapipe_name in name_idx_map:
            ...
            position = {
                "x": frame_keypoints[keypoint_idx][0] * 50,
                "y": frame_keypoints[keypoint_idx][1] * 50,
                "z": frame_keypoints[keypoint_idx][2] * 50
            }

    return position


def convert_keypoints_to_mixamo_format(dat_filepath,fps, animation_name="ConvertedAnimation"):
    """
    Convert 3D keypoints data from a .dat (JSON) file to the Mixamo format JSON file structure.

    Parameters:
    dat_filepath (str): Path to the .dat file containing 3D keypoints data.
    animation_name (str): The name for the animation file.

    Returns:
    dict: JSON structure in Mixamo format
    """
    # 讀取 .dat 文件中的 3D 關鍵點數據
    with open(dat_filepath, 'r') as infile:
        json_data = json.load(infile)
    ...
    anim_result_json = {
        ...
        "duration": num_frames / fps,  # 假設 30 FPS 作為 ticksPerSecond
        "ticksPerSecond": fps,  # 默認的 ticksPerSecond 為 30
        "frames": []
    }
    ...
    for frame_idx, frame_data in enumerate(frames_data):
        ...
        bones_json = {
            "time": frame_idx / 30,  # 假設 30 FPS 作為 ticksPerSecond
            "bones": []
        }
        ...
        for bone in mixamo_names:
            ...
            bones_json["bones"].append({
                "name": bone_name,
                "position": position,
                "rotation": {"x": 0.0, "y": 0.0, "z": 0.0, "w": 1.0}  # 默認的旋轉
            })
...
def main(args):
    ...
    ############################################################################
    print("=Writing keypoints to disk...")
    ...
    #write to json
    # keypoint_path = motion_folder + '/kpts_3d_{}.json'.format(args["type"])
    # write_keypoints_to_disk(keypoint_path,landmark_names, kpts_3d)
    ...
    anim_json = convert_keypoints_to_mixamo_format(keypoint_path,fps, animation_name)
